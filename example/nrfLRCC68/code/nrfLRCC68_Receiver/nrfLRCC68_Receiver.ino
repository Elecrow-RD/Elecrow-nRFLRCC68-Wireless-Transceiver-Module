...
void setup() {
  ...
  SPI.begin();
  int state = radio.begin(915.0, 125.0, 7, 7,RADIOLIB_SX126X_SYNC_WORD_PUBLIC , 22, 8, 3.3, false);
...
void loop() {
  ...
  if(receivedFlag) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      Serial.println(F("[SX1262/SX1268] Received packet!"));

      // print data of the packet
      Serial.print(F("[SX1262/SX1268] Data:\t\t"));
      Serial.println(str);

      parseSensorData(str);

      // print RSSI (Received Signal Strength Indicator)
      Serial.print(F("[SX1262/SX1268] RSSI:\t\t"));
